...
from . import database, jobs, utils
from .model import BadMatchFilter, SiteBanner, User
...
login_required = cast(Callable[[F], F], flask_login.login_required)


def assert_user_is_admin() -> None:
...
def admin_index() -> str:
    ...
    return render_template("admin/index.html", admin_pages=admin_pages)


@admin_blueprint.route("/admin/banner")
@login_required
def admin_site_banner() -> str:
    """Adjust site banner."""
    assert_user_is_admin()
...
def admin_bad_match() -> Response | str:
    if request.method == "POST":
        item = BadMatchFilter(
            wikidata=request.form["wikidata"], osm=request.form["osm"]
        )
...
def list_active_jobs() -> str:
    ...
    return render_template(
        "admin/active_jobs.html", admin_job_lists=admin_job_lists, items=job_list
    )


@admin_blueprint.route("/admin/stop/<osm_type>/<int:osm_id>", methods=["GET", "POST"])
@login_required
def stop_job(osm_type: str, osm_id: int) -> Response | str:
    """Stop a job."""
    assert_user_is_admin()
...
def view_log(osm_type: str, osm_id: int, start) -> str:
    assert_user_is_admin()
...
@login_required
def list_slow_jobs() -> str:
    assert_user_is_admin()
    duration = PlaceMatcher.end - PlaceMatcher.start
    jobs = (
        PlaceMatcher.query.filter(PlaceMatcher.end.isnot(None))
