...
select place.place_id, place.osm_type, place.osm_id, place.added, size, display_name, state, count(changeset.id), max(place_matcher.start), username
from place
    left outer join
...
place_matcher ON place_matcher.osm_id = place.osm_id and place_matcher.osm_type = place.osm_type
    left outer join
"user" on place_matcher.user_id = "user".id, (
...
group by place.place_id, place.added, display_name, state, size, username order by size desc;'''
