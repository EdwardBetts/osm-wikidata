...
def export_place(osm_type, osm_id):
    ...
    fields = ['place_id', 'osm_type', 'osm_id', 'display_name', 'category', 'type',
              'place_rank', 'icon', 'south', 'west', 'north', 'east', 'extratags',
              'address', 'namedetails', 'item_count', 'candidate_count', 'state',
              'override_name', 'lat', 'lon', 'wikidata_query_timeout', 'wikidata',
              'item_types_retrieved', 'index_hide', 'overpass_is_in',
              'existing_wikidata', 'osm_url', 'type_label', 'area',
              'name_for_changeset']

    item_fields = ['item_id', 'enwiki', 'entity', 'categories', 'qid', 'query_label',
                   'ewkt', 'extract_names', 'wikidata_uri']
    ...
    place_data = {key: getattr(place, key) for key in fields}
